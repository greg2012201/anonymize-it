...
async function base64ToImageElement(base64Image: string): Promise<HTMLImageElement> {
...
function compareImages(exampleDescriptor: Float32Array, targetDescriptors: Float32Array[]) {
    const threshold = 0.6;
    return targetDescriptors.filter((targetDescriptor) => {
        const distance = faceapi.euclideanDistance(exampleDescriptor, targetDescriptor);
...
function useFace() {
    ...
    const handleFace = async () => {
        ...
        try {
            ...
            const matchedDescriptors = compareImages(exampleFace.descriptor, flattenedDescriptors);
            ...
            const matchingImageUrls = targetImages.filter((_, index) => {
                ...
                return descriptorsForImage.some((descriptor) =>
                    matchedDescriptors.some((matchedDescriptor) => matchedDescriptor === descriptor)
                );
            });
